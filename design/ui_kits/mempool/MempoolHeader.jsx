function MempoolHeader({ block, gas, baseFee, pending, gasTrend }) {
  const Stat = ({ label, value, sub, accent }) => (
    <div style={{ flex: 1, padding: '12px 16px', borderRight: '1px solid #1A1F25', minWidth: 0 }}>
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 28,
                    letterSpacing: '0.02em', color: accent || '#FFFFFF', lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ marginTop: 6, fontFamily: 'var(--font-mono)', fontSize: 11, color: '#949594' }}>{sub}</div>}
    </div>
  );
  return (
...
window.MempoolHeader = MempoolHeader;
