...
function FeeGrid({ buckets }) {
  return (
    <div style={{ background: '#1E2329', padding: 16, borderRight: '1px solid #1A1F25',
                  display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 14 }}>
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14,
                       letterSpacing: '0.18em', textTransform: 'uppercase', color: '#FFFFFF', whiteSpace: 'nowrap', paddingRight: 4 }}>
          Pending · by fee
        </span>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#949594', whiteSpace: 'nowrap' }}>
          {buckets.reduce((a, b) => a + b.count, 0).toLocaleString()} tx
        </span>
      </div>
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)',
                    gridAutoRows: '1fr', gap: 4, minHeight: 0 }}>
        {buckets.map((b, i) => {
          ...
          return (
            <div key={i} style={{ background: bg, color: fg, padding: 10,
              ...
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 22,
                              letterSpacing: '0.02em', lineHeight: 1 }}>{b.count.toLocaleString()}</div>
